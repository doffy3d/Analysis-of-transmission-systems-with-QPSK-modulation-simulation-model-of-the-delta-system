...
%     
%% 1. Generisanje informacione sekvence - diskretan iyvor bey memorije     
Nsim = 100000;               % broj generisanih bita
xx = rand(Nsim,1);           % generisanje informacione binarne sekvence
...
%% zastitni koder
x = encode(xx,7,4,'hamming/binary');
...
% filtriranje signala
[I_grana_fir, zf1] = filter(h_fir, 1, I_grana_up);
[Q_grana_fir, zf2] = filter(h_fir, 1, Q_grana_up);
...
%% ABGS - kanal
Ebpn = 15;           %odnos srednje bitske energije i SGSS šuma u dB
...
[I_grana_prijem_fir, zf1 ] = filter(h_fir, 1, I_grana_prijem);
[Q_grana_prijem_fir, zf2 ] = filter(h_fir, 1, Q_grana_prijem);
...
[Broj_gresaka_pre,BER_pre] = biterr(x,biti_prijem)

 biti_prijem_posle = decode(biti_prijem,7,4,'hamming/binary');

[Broj_gresaka,BER] = biterr(xx',biti_prijem_posle)
